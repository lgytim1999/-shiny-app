...
proetomics_table =function(test_proteomics, test_metabolics, test_clinical){
  # Load data
  #table S1.1 covid patients clinical data
  patient_clinical <- read.csv("data/patient_clinical_data.csv",
                               stringsAsFactors = FALSE,
  ...
  fit <- eBayes(fit)
  tT <- topTable(fit, coef = 2, number = Inf, sort.by ="logFC")
  Top_100_proteins_selected_for_proetomics <- c(rownames(tT)[1:100])
  return(data.frame(Top_100_proteins_selected_for_proetomics))
}

metabolites_table =function(test_proteomics, test_metabolics, test_clinical){
  # Load data
  #table S1.1 covid patients clinical data
  patient_clinical <- read.csv("data/patient_clinical_data.csv",
                               stringsAsFactors = FALSE,
  ...
  fit <- eBayes(fit)
  tT <- topTable(fit, coef = 2, number = Inf, sort.by ="logFC")
  Top_100_proteins_selected_for_metabolites <- c(rownames(tT)[1:100])

  
  return(data.frame(Top_100_proteins_selected_for_metabolites))
}
...
return(hist(protein_mtrxy, xlab ='protein', ylab = 'score'))
}
